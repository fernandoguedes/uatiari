...
def mock_paths():
    ...
    with patch("uatiari.config.Path") as mock_path:
        ...
        }.get(x, MagicMock()) # This is actually home/.config/uatiari
...
class TestLoadConfigurations:
    ...
    def test_priority_1_local_env(self, mock_path, mock_dotenv_values, mock_load_dotenv, mock_env_vars):
        ...
        mock_dotenv_values.side_effect = lambda p: {"GOOGLE_API_KEY": "local_key"} if p == mock_local_env else {}
    ...
    @patch("uatiari.config.dotenv_values")
    @patch("uatiari.config.Path")
    def test_priority_2_user_config(self, mock_path, mock_dotenv_values, mock_load_dotenv, mock_env_vars):
        ...
        mock_path.home.return_value.__truediv__.return_value.__truediv__.return_value.__truediv__.return_value = mock_user_config_env
        
        mock_dotenv_values.side_effect = lambda p: {"GOOGLE_API_KEY": "user_key"} if p == mock_user_config_env else {}
    ...
    def test_directory_creation(self, mock_path, mock_dotenv_values, mock_load_dotenv):
        ...
        mock_path.home.return_value.__truediv__.return_value.__truediv__.return_value = mock_user_config_dir
    ...
    @patch("uatiari.config.dotenv_values")
    @patch("uatiari.config.Path")
    def test_directory_creation_error_handled(self, mock_path, mock_dotenv_values, mock_load_dotenv):
